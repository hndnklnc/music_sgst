...
<title>Nosataljik Sarkılar</title>
...
for($i=0;$i<$size-1;$i++){
$result=mysql_query("select SarkiciAdi,SarkiAdi,SarkiLink from nostaljik where SarkiTuru='$turlerArray[$i]'") ;
...
for ($row_num = 0; $row_num < $num_rows; $row_num++) {
    ...
    for ($index = 0; $index < $num_fields; $index++) {
		if($index==$num_fields-1){
			...
			print "<th>"."<form id=form2 name=form2  method=post action=sarkiCalNostaljik.php><input type=submit name='dugme' type='button' value='Dinle'/> 
			...
			"<th>"."<form id=form2 name=form2  method=post action=sarkiCalNostaljik.php><input type=submit name='dugme' type='button' value='Dinle'/> 
			<input name=sarkiYeri type=hidden value='$value'/>
			<input name=sarkiAdi type=hidden value='$value_sarki'/>
			<input name=sarkiciAdi type=hidden value='$value_sarkici'/> </form>". "</th> ";
		}else{
...
$resultD_0=mysql_query("select SarkiciAdi,SarkiAdi,SarkiLink from nostaljik where SarkiOrt<3") ;
